...
class ClienteController{
    ...
    async findClient(req, res){
        ...
        var result = await Cliente.findAllClient()
        if(result.length > 0){
            ...
            res.status(400);
            res.json({error: "Problema para buscar os clientes"})
        }
    }
    ...
    async create(req, res){
        ...
        var result = await Cliente.new(Nome, Ativo, ID_sharepoint, ID_funcionario, last_update);
